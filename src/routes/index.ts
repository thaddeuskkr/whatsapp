	...
	.ws('', {
		async beforeHandle({ query, status, store: { auth } }) {
			if (auth.length > 0) {
				const { token } = query;
				if (!token) {
					return status(400, { success: false, error: 'No WebSocket connection token provided' });
				}

				const dbToken = await Tokens.findOne({ token, expiresAt: { $gt: new Date() } });
				if (!dbToken) {
					return status(401, { success: false, error: 'Invalid or expired WebSocket connection token' });
				}
		...
		async open(ws) {
			...
			setInterval(() => {
				...
				if (!ws.id) {
					return ws.send(JSON.stringify({ op: Op.Error, message: 'Connection ID not found' }));
				}

				ws.send(JSON.stringify({ op: Op.Heartbeat, message: 'Expecting heartbeat response within 5 seconds' }));
		...
		async message(ws, message: MessageT) {
			if (typeof message !== 'object' || message === null || Array.isArray(message)) {
				return ws.send(JSON.stringify({ op: Op.Error, message: 'Message cannot be parsed as JSON' }));
			}

			if (!('op' in message) || typeof message.op !== 'number') {
				return ws.send(JSON.stringify({ op: Op.Error, message: 'Message must contain a valid operation (op) code' }));
			}
			...
			switch (message.op as Op | unknown) {
				...
				default: {
					return ws.send(JSON.stringify({ op: Op.Error, message: `Unknown operation code: ${message.op}` }));
				}
			}
		},
	});
