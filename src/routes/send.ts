...
const route = (app: Server) => app.post('', async ({ status, headers: { authorization }, store: { auth }, body, whatsapp }) => {
	...
	body: t.Object({
		...
		mentions: t.Optional(t.Array(t.String(), { description: 'An array of user IDs to mention in the message' })),
	}),
});
